...
if(!isset($_GET['pn'])){
 ...
 if($num_select_pn < 1){ 
    echo "<script>alert('kindly upload service!')</script>";
    echo "<script>window.open('index.php', '_self')</script>";
    exit(); 
 }
...
<input type="text" name="" id="myInput" value="kehkehask.com<?=$add_to_p_link;?>/service/order_page.php?buid=<?=$brand_unique_id;?>&pcn=<?=$product_name;?>">
