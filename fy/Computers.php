...
$select_pn = mysqli_query($conn, "SELECT * FROM reg_product WHERE p_category = 'Computers' AND status = '' AND member = 'FY_001_MEM' ORDER by rand() DESC");
$num_select_pn = mysqli_num_rows($select_pn);
if($num_select_pn < 1){ 
    echo "<script>alert('Computer product not avaliabe yet!')</script>";
...
<a href="product/order_page.php?buid=<?= $brand_unique_id ?>&pcn=Computers" class="link_p" target="_blank">Order now</a>
</div>
</div>
