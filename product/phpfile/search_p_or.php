...
if(isset($_POST['inp_search'])){
    ...
    $query = mysqli_query($conn, "SELECT * FROM reg_product WHERE p_name LIKE '%$inp_search%' AND status = '' AND brand_unique_id = '$brand_unique_id'");
    ...
    <div class="div_display_search">
        ...
        <input type="hidden" name="p_unique_id" value="<?= $query_row['p_unique_id'];?>">
        <input type="submit" name="btn_searched" value="<?= $query_row['p_name'];?>" class="inp_p_search"><hr>
        </form> 
        ...
        <?php }else{ ?>
    <div class="div_display_search">
        <a>no record</a>  <?= $product_category_name; ?>
    </div>  
       <?php } ?>
<?php } ?>