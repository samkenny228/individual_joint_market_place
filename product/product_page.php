...
if(!isset($_GET['pn'])){
 ...
 $product_name = htmlentities(mysqli_real_escape_string($conn, $_GET['pn']));
 $select_pn = mysqli_query($conn, "SELECT * FROM reg_product WHERE p_category = '$product_name' AND brand_unique_id = '$brand_unique_id' AND member = '$member' ORDER by 1 DESC");
 $num_select_pn = mysqli_num_rows($select_pn);
 if($num_select_pn < 1){ 
    echo "<script>alert('kindly upload product!')</script>";
    echo "<script>window.open('index.php', '_self')</script>";
    exit(); 
 }
...
<input type="text" name="" id="myInput" value="kehkehask.com/fy/product/order_page.php?buid=<?=$brand_unique_id;?>&pcn=<?=$product_name;?>">
